  ...
  public class APIErrorHandlingAnalyzer : DiagnosticAnalyzer
  {
    ...
    private bool IsValidResultMatchInvocation(InvocationExpressionSyntax invocation, out string variableName, out SwitchExpressionSyntax switchExpression)
    {
      ...
      if (invocation is null
        || !(_context.SemanticModel.GetSymbolInfo(invocation).Symbol is IMethodSymbol matchMethod)
        || matchMethod.Name != "Match" || matchMethod.ReceiverType.Name != "APIResult"
        || !matchMethod.ContainingNamespace.Name.StartsWith("osu_sharp"))
        return false;
    ...
    /// <summary>
    /// Returns whether the specified statement is an osu-sharp API call, assigning the result to the variable with the specified name.
    /// <br/><br/>
    ...
    private bool IsValidAPICall(StatementSyntax statement, string variableName, out IMethodSymbol apiCallMethod)
    {
      ...
      // Check whether a method call was found and if so, whether the returned type is an APIResult and the method from the osu-sharp namespace.
      if (apiCallInvocation == null)
        return false;
      if (!(_context.SemanticModel.GetSymbolInfo(apiCallInvocation).Symbol is IMethodSymbol method))
        return false;
      if (!method.ContainingNamespace.Name.StartsWith("osu_sharp"))
        return false;
